...
int main()
{
    ...
    void * ACCL_TOP_A;
    ACCL_TOP_A = (void*) mmap(NULL, TOP_0_ACCL_TOP_INTERNAL_INST_AVS_A_SPAN, 
                       (PROT_READ | PROT_WRITE), MAP_SHARED, 
    ...
    for(unsigned int i=0; i<ELEMENTS; i++)
    {
        ...
        alt_write_word(correct_addrA+i, array_a[i]);
        alt_write_word(correct_addrB+i, array_b[i]);
        alt_write_word(correct_addrC+i, 0);
    }
    ...
    {
        int pred = alt_read_word(correct_addrC+i);
        printf("%s%d%s%d\n", "Reference = ", ref[i], ", Hardware = ", pred);
        if(ref[i] != pred)
        {
            perror("Test failed!!!");
            break;
